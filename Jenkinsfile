        ...
        stage('Install Secret Store CSI Driver on Kubernetes') {
            steps {
                script {
                    ...
                    helm upgrade --install csi-secrets-store secrets-store-csi-driver/secrets-store-csi-driver --namespace kube-system --set syncSecret.enabled=true --kubeconfig $KUBECONFIG
