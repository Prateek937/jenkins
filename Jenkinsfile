        ...
        stage('Install Secret Store CSI Driver on Kubernetes') {
            steps {
                script {
                    ...
                    helm upgrade --install csi-secrets-store secrets-store-csi-driver/secrets-store-csi-driver --namespace kube-system --set enableSecretRotation=true --set rotationPollInterval=20s --kubeconfig $KUBECONFIG
                    ...
                    eksctl delete iamserviceaccount --name csi-sa --region=${AWS_REGION} --cluster ${CLUSTER_NAME} --approve
